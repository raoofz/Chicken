...
const urgencyLabel = (u: string) => {
  const m: Record<string, string> = {
    immediate: "الآن فوراً 🔴",
    today: "اليوم 🟠",
    this_week: "هذا الأسبوع 🟡",
    monitor: "راقب باستمرار 🔵",
  };
  return m[u] ?? "—";
};
...
const riskLabel = (l: string) => {
  const m: Record<string, string> = {
    critical: "خطر كبير 🔴", high: "خطر مرتفع 🟠",
    medium: "خطر متوسط 🟡", low: "وضع جيد 🟢",
  };
  return m[l] ?? "—";
};
...
function PredictiveView({ result }: { result: AdvancedOutput }) {
  return (
    <div className="space-y-5">
      ...
      {/* 1. What we see now — simple observations */}
      <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 space-y-2 text-right">
        <div className="flex items-center gap-2 mb-2">
          <Activity className="w-4 h-4 text-blue-600" />
          <span className="font-bold text-sm text-blue-800">ماذا نرى في مزرعتك الآن</span>
        </div>
      ...
      {/* 2. Risk level — clear and simple */}
      <div>
        <div className="flex items-center gap-2 mb-2">
          <AlertTriangle className="w-4 h-4 text-amber-600" />
          <span className="font-bold text-sm">مستوى الخطر</span>
        </div>
      ...
      <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 space-y-3 text-right">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="font-bold text-sm text-indigo-800">ماذا يُتوقع أن يحدث</span>
        </div>
        ...
        <div className="flex items-center gap-3 text-sm">
          <span className="bg-indigo-200 text-indigo-800 px-2 py-0.5 rounded-lg font-bold">
            احتمال: {result.prediction.probability}%
          </span>
          <span className="text-muted-foreground">{result.prediction.timeHorizon}</span>
        </div>
        {result.prediction.scenarios?.length > 0 && (
          <div className="pt-2 border-t border-indigo-200">
            <div className="text-xs font-bold text-indigo-600 mb-2">السيناريوهات المتوقعة:</div>
            <ScenarioBar scenarios={result.prediction.scenarios} />
          </div>
        )}
      </div>

      {/* 4. What to do — action plan — most important section */}
      <div>
        <div className="flex items-center gap-2 mb-3">
          <Lightbulb className="w-4 h-4 text-emerald-600" />
          <span className="font-bold text-sm">ماذا تفعل الآن — بالترتيب</span>
        </div>
        <ActionPlanList steps={result.actionPlan} />
      </div>

      {/* 5. Impact — what happens if you don't act */}
      <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4 space-y-2 text-right">
        <div className="flex items-center gap-2 mb-1">
          <AlertCircle className="w-4 h-4 text-orange-600" />
          <span className="font-bold text-sm text-orange-800">إذا لم تتصرف — ماذا سيحدث</span>
        </div>
        {[
          ["الآن",         result.impact.immediate],
          ["هذا الأسبوع",  result.impact.shortTerm],
          ["على المدى البعيد", result.impact.longTerm],
        ].map(([k, v], i) => (
        ...
        {result.impact.quantifiedLoss && (
          <div className="mt-2 pt-2 border-t border-orange-200 text-sm font-bold text-orange-800">
            💰 الخسارة المتوقعة: {result.impact.quantifiedLoss}
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-right space-y-2">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="font-bold text-sm text-violet-800">السبب الرئيسي</span>
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-right text-xs text-muted-foreground">
        <span className="font-bold">مصداقية التحليل: {result.confidenceScore}%</span>
        {" "} — مبني على {result.dataQuality.sampleSize} دورة سابقة، اكتمال البيانات {result.dataQuality.completeness}%
      </div>
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-bold text-sm">أهم 3 خطوات تفعلها الآن</span>
        </div>
      ...
      {result.predictive && (
        <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-right space-y-2">
          <div className="flex items-center gap-2 mb-1">
            <TrendingUp className="w-4 h-4 text-indigo-600" />
            <span className="font-bold text-sm text-indigo-800">ماذا سيحدث قريباً</span>
          </div>
...
const MODULES = [
  {
    ...
    desc: "يخبرك بما سيحدث في مزرعتك خلال الأيام القادمة — بلغة واضحة وخطوات عملية مباشرة",
    tags: ["توقعات الأيام القادمة", "أسباب المشاكل", "خطة عمل واضحة", "تحذيرات مبكرة"],
  ...
  },
  {
    ...
    label: "محرك القرار الشامل",
    desc: "يدمج كل بيانات المزرعة ويعطيك قرار واحد واضح — ماذا تفعل الآن بالأولوية",
    tags: ["قرار واحد جاهز", "أولويات فورية", "ملخص شامل", "توصية نهائية"],
...
export default function AdvancedAnalysis() {
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
        ...
        <h2 className="text-xl font-bold">للمديرين فقط</h2>
      </div>
    );
  }
  ...
  const runModule = async (mod: NonNullable<Module>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json()).error ?? "خطأ في التحليل");
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] md:h-[calc(100vh-5rem)]">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold leading-tight">الذكاء الاصطناعي المتقدم</h1>
            <p className="text-xs text-muted-foreground">تحليل مبني على بياناتك الفعلية — بلغة الفلاح</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto pb-4">
        ...
        {!module && !loading && (
          <div className="space-y-6">
            <div className="text-center space-y-2 py-4">
              ...
              <h2 className="text-lg font-bold">اختر نوع التحليل</h2>
              <p className="text-sm text-muted-foreground max-w-sm mx-auto">
                يقرأ كل بيانات مزرعتك ويعطيك إجابات واضحة — كأنك تسأل خبير بجانبك
              </p>
            </div>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 space-y-5">
            ...
            <p className="font-bold text-lg">يقرأ بيانات مزرعتك...</p>
            <p className="text-sm text-violet-500 animate-pulse">يحسب النتائج ويجهّز التوصيات...</p>
          </div>
        )}
        ...
        {result && module === "predict" && !loading && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <h2 className="font-bold text-right">التحليل التنبؤي</h2>
            </div>
        ...
        {result && module === "decision" && !loading && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <h2 className="font-bold text-right">محرك القرار الشامل</h2>
            </div>
