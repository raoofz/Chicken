...
const PRIORITY_LABELS: Record<string, string> = { low: "Låg", medium: "Medel", high: "Hög" };
...
function TaskForm({ initial, onSubmit, onClose, isEdit }: { initial?: any; onSubmit: (d: any) => void; onClose: () => void; isEdit?: boolean }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div className="space-y-1.5"><Label>Titel *</Label><Input value={form.title} onChange={(e) => setForm(f => ({ ...f, title: e.target.value }))} placeholder="T.ex. Morgonmatning" required autoFocus /></div>
      <div className="space-y-1.5"><Label>Beskrivning (valfritt)</Label><Input value={form.description} onChange={(e) => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Mer information..." /></div>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5"><Label>Prioritet</Label>
          <Select value={form.priority} onValueChange={(v) => setForm(f => ({ ...f, priority: v }))}><SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent><SelectItem value="high">Hög</SelectItem><SelectItem value="medium">Medel</SelectItem><SelectItem value="low">Låg</SelectItem></SelectContent>
          </Select>
        </div>
      </div>
      <div className="space-y-1.5"><Label>Datum</Label><Input type="date" value={form.dueDate} onChange={(e) => setForm(f => ({ ...f, dueDate: e.target.value }))} /></div>
      <div className="flex gap-2 justify-end"><Button type="button" variant="outline" onClick={onClose}>Avbryt</Button><Button type="submit">{isEdit ? "Spara" : "Lägg till"}</Button></div>
    </form>
  );
}
...
export default function Tasks() {
  ...
  const handleUpdate = async (data: any) => { try { await updateTask.mutateAsync({ id: editItem.id, data }); toast({ title: "Uppgift sparad" }); setEditItem(null); refresh(); } catch (err: any) { toast({ title: "Fel", description: err?.message, variant: "destructive" }); } };
  ...
  const toggleComplete = async (task: any) => { try { await updateTask.mutateAsync({ id: task.id, data: { completed: !task.completed } }); refresh(); } catch { toast({ title: "Kunde inte uppdatera", variant: "destructive" }); } };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Uppgifter</h1><p className="text-muted-foreground text-sm">{dateLabel} — {done.length} av {tasks?.length ?? 0} klara</p></div>
        {isAdmin && (
          <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="gap-2"><Plus className="w-4 h-4" />Ny uppgift</Button></DialogTrigger>
            <DialogContent><DialogHeader><DialogTitle>Ny uppgift</DialogTitle></DialogHeader><TaskForm onSubmit={handleCreate} onClose={() => setOpen(false)} /></DialogContent>
          </Dialog>
        )}
      </div>
      ...
      {tasks && tasks.length > 0 && (
        <div className="bg-card rounded-xl p-4 border border-border/60">
          ...
          <div className="flex justify-between text-xs text-muted-foreground mt-1.5"><span>{pending.length} kvar</span><span>{done.length} klara</span></div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-muted-foreground text-sm">{isAdmin ? "Lägg till nya uppgifter" : "Inga uppgifter idag"}</p>
        </CardContent></Card>
      ) : (
        <div className="space-y-6">
          {pending.length > 0 && (
            <div className="space-y-2">
              <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Att göra ({pending.length})</h2>
              {pending.map(task => (
          ...
          {done.length > 0 && (
            <div className="space-y-2">
              <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Klara ({done.length})</h2>
              {done.map(task => (
      ...
      <Dialog open={!!editItem} onOpenChange={(v) => !v && setEditItem(null)}>
        <DialogContent><DialogHeader><DialogTitle>Ändra uppgift</DialogTitle></DialogHeader>{editItem && <TaskForm initial={editItem} onSubmit={handleUpdate} onClose={() => setEditItem(null)} isEdit />}</DialogContent>
      </Dialog>
      <AlertDialog open={deleteId != null} onOpenChange={(v) => !v && setDeleteId(null)}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Ta bort uppgift?</AlertDialogTitle><AlertDialogDescription>Är du säker? Detta går inte att ångra.</AlertDialogDescription></AlertDialogHeader>
