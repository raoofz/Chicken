...
const STATUS_LABELS: Record<string, string> = { incubating: "Ruvning", hatching: "Kl\u00e4cks nu", completed: "Slutf\u00f6rd", failed: "Misslyckad" };
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSubmit({ ...form, eggsHatched: toNum(String(form.eggsHatched)), temperature: toNum(String(form.temperature)), humidity: toNum(String(form.humidity)), lockdownTemperature: toNum(String(form.lockdownTemperature)), lockdownHumidity: toNum(String(form.lockdownHumidity)), actualHatchDate: toStr(form.actualHatchDate), setTime: toStr(form.setTime), lockdownDate: toStr(form.lockdownDate), lockdownTime: toStr(form.lockdownTime) }); }} className="space-y-5 max-h-[70vh] overflow-y-auto pr-1">
      <div className="space-y-3">
        <div className="space-y-1.5"><Label>Omg\u00e5ngsnamn</Label><Input value={form.batchName} onChange={e => setForm(f => ({ ...f, batchName: e.target.value }))} placeholder="T.ex. Omg\u00e5ng 3" required /></div>
          ...
          <div className="space-y-1.5"><Label>Status</Label>
            <Select value={form.status} onValueChange={v => setForm(f => ({ ...f, status: v }))}>
              ...
              <SelectContent>
                ...
                <SelectItem value="completed">Slutf\u00f6rd</SelectItem><SelectItem value="failed">Misslyckad</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-blue-200 bg-blue-50/60 p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label>\u00c4gg ins\u00e4ttningsdatum</Label><Input type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} required /></div>
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4 space-y-3">
        <div className="flex items-center gap-2 text-amber-700 font-semibold text-sm"><ArrowLeftRight className="w-4 h-4" />Fas 2 \u2014 Lockdown & kl\u00e4ckning (dag 18 \u2192 21)</div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label>Flytt datum (dag 18)</Label><Input type="date" value={form.lockdownDate} onChange={e => setForm(f => ({ ...f, lockdownDate: e.target.value }))} /></div>
      ...
      <div className="rounded-xl border border-border/60 bg-muted/30 p-4 space-y-3">
        <div className="flex items-center gap-2 text-muted-foreground font-semibold text-sm"><Egg className="w-4 h-4" />Kl\u00e4ckningsresultat</div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5"><Label>F\u00f6rv\u00e4ntat kl\u00e4ckdatum</Label><Input type="date" value={form.expectedHatchDate} onChange={e => setForm(f => ({ ...f, expectedHatchDate: e.target.value }))} required /></div>
          ...
          <div className="col-span-2 space-y-1.5"><Label>Antal kl\u00e4ckta (valfritt)</Label><Input type="number" min={0} value={form.eggsHatched} onChange={e => setForm(f => ({ ...f, eggsHatched: e.target.value }))} placeholder="Anges efter slutf\u00f6rd omg\u00e5ng" /></div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Kl\u00e4ckningscykler</h1>
          <p className="text-muted-foreground text-sm">21-dagarscykel \u2014 18 dagars ruvning + 3 dagars kl\u00e4ckning</p>
        </div>
        {isAdmin && (
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button className="gap-2"><Plus className="w-4 h-4" />Ny omg\u00e5ng</Button></DialogTrigger>
            <DialogContent className="max-w-lg">
              <DialogHeader><DialogTitle>L\u00e4gg till ny kl\u00e4ckningsomg\u00e5ng</DialogTitle></DialogHeader>
              <CycleForm onSubmit={async d => { try { await createCycle.mutateAsync({ data: d }); toast({ title: "Omg\u00e5ng tillagd" }); setOpen(false); refresh(); } catch (err: any) { toast({ title: "Fel", description: err?.message, variant: "destructive" }); } }} onClose={() => setOpen(false)} />
            </DialogContent>
          </Dialog>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs">
        ...
        <div className="flex items-center gap-1.5 bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg border border-amber-200"><ArrowLeftRight className="w-3.5 h-3.5" />Dag 18\u201321: Lockdown \u2014 37.0\u201337.5\u00b0C / fuktighet 65\u201375%</div>
      </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-3 bg-amber-50/60 rounded-lg px-3 py-2 border border-amber-100">
                        <span className="text-xs font-medium text-amber-700">Lockdown (18\u201321):</span>
                        {cycle.lockdownDate ? <span className="text-xs text-muted-foreground flex items-center gap-1">{cycle.lockdownDate}{cycle.lockdownTime && <span className="flex items-center gap-0.5 ml-1"><Clock className="w-3 h-3" />{cycle.lockdownTime}</span>}</span> : <span className="text-xs text-muted-foreground/50">\u00c4nnu ej flyttad</span>}
                        ...
                        <span className="text-xs text-muted-foreground">F\u00f6rv\u00e4ntat: {cycle.expectedHatchDate}</span>
      ...
      <Dialog open={!!editItem} onOpenChange={v => !v && setEditItem(null)}>
        <DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Redigera omg\u00e5ng</DialogTitle></DialogHeader>
      ...
      <AlertDialog open={deleteId != null} onOpenChange={v => !v && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>Bekr\u00e4fta borttagning</AlertDialogTitle><AlertDialogDescription>\u00c4r du s\u00e4ker p\u00e5 att du vill ta bort denna omg\u00e5ng? Detta kan inte \u00e5ngras.</AlertDialogDescription></AlertDialogHeader>
