...
const SECTIONS = [
  ...
  { key: "📊 تحليل الأداء العام", icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-200", title: "Prestationsanalys" },
  { key: "🔮 توقعات الأسبوعين القادمين", icon: Sparkles, color: "text-purple-600", bg: "bg-purple-50 border-purple-200", title: "Prognoser (2 veckor)" },
  { key: "⚡ خطة العمل الفورية (أولويات الأسبوع)", icon: Zap, color: "text-orange-600", bg: "bg-orange-50 border-orange-200", title: "Veckans handlingsplan" },
...
export default function AiInsights() {
  ...
  if (!isAdmin) {
    return <div className="flex flex-col items-center justify-center py-24 text-center"><AlertTriangle className="w-16 h-16 text-amber-500 mb-4" /><h2 className="text-xl font-bold mb-2">Begränsad åtkomst</h2><p className="text-muted-foreground">Denna sida är bara tillgänglig för administratörer</p></div>;
  }
  ...
  const analyze = async () => {
    ...
    catch (err: any) { setError(err.message ?? "Ett fel uppstod"); }
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">Analys av kläckning, flockar, uppgifter, mål, anteckningar och prognoser</p>
        </div>
        <Button onClick={analyze} disabled={loading} className="gap-2 shadow-md" size="lg">
          ...
          {loading ? "Analyserar..." : result ? "Uppdatera analys" : "Starta analys"}
        </Button>
      </div>
      ...
      {!result && !loading && (
        <Card className="border-primary/20 bg-gradient-to-br from-primary/8 to-amber-50">
          <CardContent className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-muted-foreground text-sm max-w-lg leading-relaxed mb-2">Analyserar all gårdsdata: temperatur och luftfuktighet i kläckmaskinen, flockhälsa, försenade uppgifter, målframsteg och dagliga anteckningar</p>
            <p className="text-muted-foreground text-sm max-w-lg leading-relaxed mb-6">Ger dig <strong>brådskande varningar, prognoser och en prioriterad handlingsplan</strong></p>
      ...
      {loading && (
        <div className="space-y-4">
          <div className="text-center text-muted-foreground flex items-center justify-center gap-2 py-4"><RefreshCw className="w-5 h-5 animate-spin text-primary" /><span className="font-medium">AI analyserar din data...</span></div>
      ...
      {result && !loading && (
        <div className="space-y-4">
          ...
          <Card className="border-border/50"><CardContent className="p-0">
            <button onClick={() => setShowRaw(!showRaw)} className="w-full p-4 flex items-center justify-between text-sm text-muted-foreground hover:text-foreground transition-colors"><span className="flex items-center gap-2"><TrendingUp className="w-4 h-4" />Data som användes i analysen</span>{showRaw ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</button>
