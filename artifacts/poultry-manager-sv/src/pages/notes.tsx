...
export default function Notes() {
  ...
  const addMutation = useMutation({ mutationFn: createNote, onSuccess: () => { toast({ title: "Anteckning sparad" }); qc.invalidateQueries({ queryKey: ["notes"] }); setContent(""); setOpen(false); }, onError: () => toast({ title: "Fel", description: "Kunde inte spara", variant: "destructive" }) });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Anteckningar</h1><p className="text-muted-foreground text-sm">Dokumentera observationer och händelser</p></div>
        {isAdmin && (
          <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="gap-2"><Plus className="w-4 h-4" />Ny anteckning</Button></DialogTrigger>
            <DialogContent><DialogHeader><DialogTitle>Ny anteckning</DialogTitle></DialogHeader>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-muted-foreground text-sm">{isAdmin ? "Börja dokumentera dina observationer" : "Inga anteckningar har lagts till ännu"}</p>
        </CardContent></Card>
      ) : (
      ...
      <AlertDialog open={deleteId != null} onOpenChange={(v) => !v && setDeleteId(null)}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Ta bort anteckning?</AlertDialogTitle><AlertDialogDescription>Är du säker? Detta går inte att ångra.</AlertDialogDescription></AlertDialogHeader>
