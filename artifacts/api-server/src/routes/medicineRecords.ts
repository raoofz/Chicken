...
router.post("/medicine-records", async (req, res) => {
  try {
    const { date, flockId, batchId, medicineName, dosage, cost, notes, createTransaction } = req.body;
    if (!date || !medicineName || cost === undefined) {
      res.status(400).json({ error: "date, medicineName, cost are required" });
      return;
    }

    // Atomically: insert the medicine record and (optionally) a linked expense transaction.
